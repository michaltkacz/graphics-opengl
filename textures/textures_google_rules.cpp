...
const std::string TEXTURE_FILES[] = {
    "D1_t.tga", "D2_t.tga", "D3_t.tga", "D4_t.tga", "D5_t.tga", "D7_t.tga",
    "D8_t.tga", "D9_t.tga", "M1_t.tga", "M2_t.tga", "M3_t.tga", "M4_t.tga",
    "N1_t.tga", "N2_t.tga", "N3_t.tga", "P1_t.tga", "P2_t.tga", "P3_t.tga",
...
GLbyte* load_tga_image(const char* file_name, GLint* image_width,
                       GLint* image_height, GLint* image_components,
                       GLenum* image_format);
...
void mouse_motion(GLsizei x, GLsizei y) {
  ...
  if (mbc == left) {
    ...
    if (observer_radius >= 30)
      observer_radius = 30.0;
    else if (observer_radius <= 1.0)
      observer_radius = 1.0;
  }
...
void my_init() {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      egg_texture_map[i][j][0] = i / (static_cast<float>(N - 1));
      egg_texture_map[i][j][1] = j / (static_cast<float>(N - 1));
    }
  }
...
void load_random_texture() {
  std::string file_name =
      "textures/" + TEXTURE_FILES[rand() % TEXTURE_FILES_NUMBER];
...
void load_texture(std::string file_name) {
  ...
  GLbyte* image_bytes =
      load_tga_image(file_name.c_str(), &image_width, &image_height,
                     &image_components, &image_format);
  glTexImage2D(GL_TEXTURE_2D, 0, image_components, image_width, image_height, 0,
               image_format, GL_UNSIGNED_BYTE, image_bytes);
  free(image_bytes);
}
...
#pragma pack(8)
  ...
  *image_height = tga_header.height;
  ...
  if (tga_header.bitsperpixel != 8 && tga_header.bitsperpixel != 24 &&
  ...
  // Memory allocation.
  GLbyte* bits_per_pixel =
      reinterpret_cast<GLbyte*>(malloc(image_size * sizeof(GLbyte)));
